Given('the user has permission to register new employees') do
  ...
  When('the user submits a valid registration form for a new employee') do
    @create_employee = @create.create_employee(DATABASE[:employee][:name], DATABASE[:employee][:roleId], DATABASE[:employee][:entryDate])
  end
  
  Then('the system should successfully register the new employee') do
    @assert.create_success(@create_employee.code, @create_employee.message)
  end
  ...
  When('the user submits valid updates for an existing employee') do
    @update_employee = @update.update_employee(DATABASE[:employee][:id], DATABASE[:employee][:name], DATABASE[:employee][:roleId], DATABASE[:employee][:entryDate])
  end
  ...
  When('the user requests the details of an employee by their ID') do
    @get_employee_by_id = @get_by_id.get_employee_by_id(DATABASE[:employee][:id])
  end
  ...
  Given('the user has permission to delete employee accounts') do
   ...
   When('the user initiates the deletion of an employee record') do
     @delete_employee = @delete.delete_employee(DATABASE[:employee][:id])
   end
